...
import tracker.domain.goal.{ CreateGoal, Goal, Id, Progress, State, Timeline }
...
object Goals:
  def makeInMem[F[_]: Functor: GenUUID](data: Ref[F, Map[user.Id, List[Goal]]]): Goals[F] = new Goals[F]:
    ...
    def create(createReq: CreateGoal): F[Goal] =
      ...
      id.map(id =>
        Goal(id, createReq.name, createReq.description, State.Created, Timeline(now, now, None), Progress(0), Nil)
      )
